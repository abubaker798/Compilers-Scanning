...
void getNextToken(CompilerInfo* files, Token* ptoken)
{

    int i;
    char* buffer = files-> in_file.GetNextTokenStr(); // set buffer form input file
    if(!buffer)
    {
        ptoken->type=ENDFILE;
        ptoken->str[0]=0;
    ...
    {
        if(StartsWith(buffer, symbolic_tokens[i].str))
            break;
    }
    ...
    if(i<num_symbolic_tokens)
    {
        ...
        {
            files->in_file.Advance(strlen(symbolic_tokens[i].str));
            if(!files->in_file.SkipUpto(symbolic_tokens[i+1].str)) return;
            return getNextToken(files, ptoken);
        }
    ...
    else if(IsDigit(buffer[0]))
    {
        ...
        ptoken->type=NUM;
        Copy(ptoken->str, buffer, j);
    }
    else if(IsLetterOrUnderscore(buffer[0]))
    {
        int j=1;
        while(IsLetterOrUnderscore(buffer[j])) j++;
    ...
    if(len>0) files->in_file.Advance(len);
}

////////////////////////////////////////////////////////////////////////////////////
// Scanner only ////////////////////////////////////////////////////////////////////

void scanner(CompilerInfo* files, Token token)
{
    while(true)
    {
        getNextToken(files, &token);     

        fprintf( files->out_file.file,"[%d] %s (%s)\n", files->in_file.cur_line_num, token.str, TokenTypeStr[token.type]);
...
int main()
{
    ...
    char* debug = "debug.txt";

    CompilerInfo compiler( input,output, debug);
    Token token;

    scanner(&compiler, token);
}